...
const settings = require("../packages/server/config/settings.js");

const dbName = settings.DB_NAME;
const dbUser = settings.DB_USER;
const dbPassword = settings.DB_PASSWORD;
const dbPort = settings.DB_PORT;

if (!dbName || !dbUser || !dbPassword || !dbPort) {
  console.error(
    "DB_NAME, DB_USER, DB_PASSWORD, and DB_PORT must be set in the environment variables."
  );
  process.exit(1);
}
...
exec(
  'docker ps -a --filter "name=pg" --format "{{.Names}}"',
  (err, stdout, stderr) => {
    if (err) {
    ...
    if (containerNames.includes("pg")) {
      exec(
        'docker inspect -f "{{.State.Running}}" pg',
        (err, stdout, stderr) => {
          if (err) {
