...
export default function QuestionManager() {
  ...
  const currentSection = currentForm?.sections?.find(
  ...
  const handleEdit = (question) => {
    ...
    setEditForm({
      ...
      multiple_choice_answers:
        question.multiple_choice_answers?.filter((a) => a !== null) || [],
      newOption: "",
    });
  };
  ...
  const handleSave = async () => {
    try {
      await updateQuestion(editingQuestionId, {
        ...editForm,
        multiple_choice_options: editForm.multiple_choice_answers.map(
          (a) => a.answer
        ),
      });
  ...
  const handleAddNewOption = (e) => {
    ...
    if (newQuestion.newOption.trim()) {
      setNewQuestion({
        ...newQuestion,
        multiple_choice_options: [
          ...newQuestion.multiple_choice_options,
          newQuestion.newOption.trim(),
        ],
        newOption: "",
      });
    }
  ...
  const handleRemoveNewOption = (indexToRemove) => {
    setNewQuestion({
      ...newQuestion,
      multiple_choice_options: newQuestion.multiple_choice_options.filter(
              ...
              <Card.Body>
                {editingQuestionId === question.id ? (
                  <Form>
                    ...
                    {(editForm.answer_type === "multiple_choice" ||
                      editForm.answer_type === "dropdown") && (
                          ...
                          {editForm.multiple_choice_answers.map((option) => (
                            <li key={option.id}>
                              <Form.Control
                                ...
                                value={option.answer}
                                onChange={(e) =>
                                  handleEditOption(option.id, e.target.value)
                                }
                              />
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleRemoveEditOption(option.id)
                                }
                              >
                                Remove
                              </Button>
                  ...
                  <>
                    ...
                    <p>
                      <strong>Type:</strong> {question.answer_type}
                    </p>
        ...
        <Card.Body>
          ...
          <Form onSubmit={handleCreate}>
            ...
            {(newQuestion.answer_type === "multiple_choice" ||
              newQuestion.answer_type === "dropdown") && (
              <div className="multiple-choice-section">
                <h4>Multiple Choice Options:</h4>
                <Form.Group className="mb-3 d-flex gap-2">
                  <Form.Control
                ...
                {newQuestion.multiple_choice_options.length > 0 && (
                  <ul className="options-list">
                    {newQuestion.multiple_choice_options.map(
                      (option, index) => (
                        <li key={index}>
                          <span>{option}</span>
                          <Button
